  ...
  'vue/no-undef-components': [
    {
      ignorePatterns: [
        ...
        '^VueForm$',
        '^Validate$',
        '^FieldMessages$', // VueForm components
      ],
    },
  ],
...
const disabledRules = [
  ...
  'unicorn/consistent-class-member-order', // cosmetic ordering that would separate private helpers from their callers
  'unicorn/consistent-function-scoping',
  'unicorn/default-export-style', // `export default class X {}` breaks jsdoc2md class detection in `build:model-docs`
  'unicorn/filename-case',
  'unicorn/no-computed-property-existence-check', // dynamic `key in obj` checks are fine and widely used
  'unicorn/no-this-outside-of-class', // needed in Vue Options API
  'unicorn/no-null',
  'unicorn/no-process-exit',
  'unicorn/no-top-level-assignment-in-function', // module-level state assigned from functions is intentional here
  'unicorn/no-top-level-side-effects', // some modules legitimately set up state at import time
  ...
  'unicorn/prefer-number-coercion', // `Number.parseInt(x, 10)` is intentional and not equivalent to `Number(x)`
  'unicorn/prefer-private-class-fields', // `#private` members are not documented by jsdoc2md in `build:model-docs`
  'unicorn/prefer-uint8array-base64', // `Uint8Array.fromBase64()`/`toBase64()` are not available in the supported Node.js/browser versions yet
...
const getRuleParameters = (ruleOptions) =>
  (Array.isArray(ruleOptions) ? ruleOptions.slice(1) : []);

for (const [vueRuleName, extendedRuleName] of Object.entries(
  vueExtensionRules,
)) {
  if (enabledRuleParameters[extendedRuleName]) {
    enabledRuleParameters[vueRuleName]
      = enabledRuleParameters[extendedRuleName];
  }
  else if (
    eslintJs.configs.recommended.rules[extendedRuleName]
    && eslintJs.configs.recommended.rules[extendedRuleName] !== 'off'
  ) {
    enabledRuleParameters[vueRuleName] = getRuleParameters(
      eslintJs.configs.recommended.rules[extendedRuleName],
    );
  }
  else if (
    stylisticEslintConfig.rules[extendedRuleName]
    && stylisticEslintConfig.rules[extendedRuleName] !== 'off'
  ) {
    enabledRuleParameters[vueRuleName] = getRuleParameters(
      stylisticEslintConfig.rules[extendedRuleName],
    );
...
export default [
  ...
  ...eslintPluginJsoncConfigs['recommended-with-json'], // has to be after `vue`
  {
    ...
    rules: {
      ...
      ...Object.fromEntries(disabledRules.map((ruleName) => [ruleName, 'off'])),
    },
    settings: {
      jsdoc: {
        tagNamePreference: {
