...
const colorHash = new ColorHash({
  ...
  hash: str => [...str].reduce((acc, char, index) => {
...
class Register {
  ...
  constructor(manufacturers, register) {
    if (register) {
      ...
      this.contributorsLastAction = {};
      this.rdm = register.rdm;
      this.colors = register.colors;

      Object.keys(this.contributors).forEach(contributor => {
        this.contributorsLastAction[contributor] = this.contributors[contributor].map(
          fixKey => this.filesystem[fixKey].lastActionDate
        ).reduce(
          (acc, curDate) => (curDate > acc ? curDate : acc),
          `0000-00-00`
        );
      });
    }
  ...
  _addFixtureToContributor(manKey, fixKey, contributor, lastActionDate) {
    if (!(contributor in this.contributors)) {
      ...
      this.contributorsLastAction[contributor] = `0000-00-00`;
    }
    this.contributors[contributor].push(`${manKey}/${fixKey}`);

    if (lastActionDate > this.contributorsLastAction[contributor]) {
      this.contributorsLastAction[contributor] = lastActionDate;
    }
  }
  ...
  getAsSortedObject() {
    ...
    const sortedContributors = Object.keys(this.contributors).sort((nameA, nameB) => {
      const lastActionA = this.contributorsLastAction[nameA];
      const lastActionB = this.contributorsLastAction[nameB];
