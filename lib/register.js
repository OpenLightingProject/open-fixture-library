...
class Register {
   ...
   */
  addFixtureRedirect(manufacturerKey, fixKey, redirectData, redirectToData) {
    this.filesystem[`${manufacturerKey}/${fixKey}`] = {
    ...
    if (redirectData.reason === `SameAsDifferentBrand`) {
      ...
      this._addFixtureToManufacturer(manufacturerKey, fixKey);

      // add to category register
      redirectToData.categories.forEach(category => {
        this._addFixtureToCategory(manufacturerKey, fixKey, category);
      });
    }
  }
   ...
   */
  addFixture(manufacturerKey, fixKey, fixData) {
    ...
    fixData.meta.authors.forEach(contributor => {
      this._addFixtureToContributor(manufacturerKey, fixKey, contributor, lastActionDate);
    });

    // add to rdm register
    if (`rdm` in fixData && `rdmId` in this._manufacturerData[manufacturerKey]) {
   ...
   * @param {String} fixKey The fixture (redirect) key.
   */
  _addFixtureToManufacturer(manufacturerKey, fixKey) {
    if (!(manufacturerKey in this.manufacturers)) {
      this.manufacturers[manufacturerKey] = [];
    }
    this.manufacturers[manufacturerKey].push(fixKey);
  }
   ...
   */
  _addFixtureToCategory(manufacturerKey, fixKey, category) {
    if (!(category in this.categories)) {
      this.categories[category] = [];
    }
    this.categories[category].push(`${manufacturerKey}/${fixKey}`);
  }
   ...
   */
  _addFixtureToContributor(manufacturerKey, fixKey, contributor, lastActionDate) {
    if (!(contributor in this.contributors)) {
      this.contributors[contributor] = {
        lastActionDate,
        fixtures: [`${manufacturerKey}/${fixKey}`],
      };
    }
    else {
      this.contributors[contributor].fixtures.push(`${manufacturerKey}/${fixKey}`);
