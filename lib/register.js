...
class Register {
  ...
  getAsSortedObject() {
    const sortedRegister = {
      ...
      manufacturers: getObjectSortedByKeys(this.manufacturers, arr => arr.sort(localeSort)),
      categories: getObjectSortedByKeys(this.categories, arr => arr.sort(localeSort)),
      contributors: {},
      rdm: {},
      lastUpdated: []
    };
    ...
    const sortedContributors = Object.keys(this.contributors).sort((a, b) => {
      const fixturesDelta = this.contributors[b].length - this.contributors[a].length;
    ...
    sortedRegister.lastUpdated = Object.keys(this.filesystem).filter(
      fixKey => `lastModifyDate` in this.filesystem[fixKey]
    ).sort((a, b) => {
      const fixA = this.filesystem[a];
      const fixB = this.filesystem[b];
 ...
 * @param {function|null} itemSortFunction A function to be invoked with the value of an object entry to sort it (useful for array values).
 * @returns {object} A new object with the same entries, sorted by keys.
 */
function getObjectSortedByKeys(obj, itemSortFunction) {
  const sortedObj = {};
  const keys = Object.keys(obj).sort(localeSort);

  for (const key of keys) {
    if (itemSortFunction) {
      sortedObj[key] = itemSortFunction(obj[key]);
    }
    else {
