 ...
 */
module.exports = async function createPullRequest(fixtureCreateResult, githubUsername = null, githubComment = null) {
  const {
    manufacturers,
    fixtures,
    warnings,
  ...
  try {
    ...
    const result = await githubClient.pulls.create({
      owner: `OpenLightingProject`,
      repo: repository,
      title: title,
  ...
  function getSubmitterNameMarkdown(user) {
    ...
    const firstFixtureKey = Object.keys(fixtures)[0];
    const firstFixtureAuthor = fixtures[firstFixtureKey].meta.authors[0];
