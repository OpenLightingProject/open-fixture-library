...
export default async function createPullRequest(fixtureCreateResult, githubUsername = null, githubComment = null) {
  ...
  try {
    ...
    const shouldSkipCi = Object.keys(fixtures).length > 0;
    ...
    const title = addedFixtures.length === 1 ? addedFixtures[0] : `Add ${addedFixtures.length} new fixtures`;
...
/**
 * @param {Readonly<Record<string, unknown>>} fixtures - The fixtures to add to the pull request.
 * @param {Readonly<Record<string, unknown>>} errors - The errors to add to the pull request.
 * @param {Readonly<Record<string, unknown>>} warnings - The warnings to add to the pull request.
 ...
 * @param {Readonly<object> | null} user - GitHub user information about the submitter, or null if they are not a GitHub user.
 * @param {boolean} [shouldSkipCi=false] - True if this commit should be excluded from the CI (continuous integration tests).
 * @returns {Promise} Promise that is resolved when the file is successfully / erroneously updated / added.
 */
async function addOrUpdateFile(filename, displayName, newContentFunction, user, shouldSkipCi = false) {
  const appendToCommitMessage = shouldSkipCi ? '\n\n[skip ci]' : '';
 ...
 * @param {Readonly<object>} object - JSON object.
