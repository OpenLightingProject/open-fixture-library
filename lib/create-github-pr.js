...
module.exports = function createPullRequest(out) {
  ...
  return githubClient.gitdata.getRef({
    ...
    .then(result => {
      ...
      return githubClient.gitdata.createRef({
        owner: `OpenLightingProject`,
        repo: repository,
    ...
    .then(() => {
      ...
      Object.entries(out.fixtures).forEach(([fixtureKey, fixture], index, arr) => {
        chain = chain.then(() => addOrUpdateFile(
          `fixtures/${fixtureKey}.json`,
          `fixture '${fixtureKey}'`,
          oldFileContent => fixtureJsonStringify(fixture),
          index !== arr.length - 1 // all but the last fixture should skip CI
        ));
      });
  ...
  function addOrUpdateFile(filename, displayName, newContentFunction, excludeFromCi = false) {
    ...
    return githubClient.repos.getContents({
      owner: `OpenLightingProject`,
      repo: repository,
      path: filename
    })
