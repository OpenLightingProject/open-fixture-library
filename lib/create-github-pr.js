...
// change this to 'ofl-test' for debugging
// const repository = 'open-fixture-library';
const repository = 'ofl-test';
...
module.exports = function createPullRequest(out, callback) {
  ...
  if (userToken === undefined) {
    console.log('environment variable GITHUB_USER_TOKEN not set, trying to read it from .env file...');
    ...
    try {
      ...
      callback('GitHub user token was not set', null);
      console.error('no .env file to read GITHUB_USER_TOKEN variable from');
      return;
    }
  }
  ...
  .then(() => {
    ...
    for (const fixtureKey in out.fixtures) {
      ...
      let manufacturerName = 'INVALID';
      if (manKey in out.manufacturers) {
        manufacturerName = out.manufacturers[manKey].name;
      }
      else if (manKey in register.manufacturers) {
        manufacturerName = register.manufacturers[manKey].name;
      }
      ...
      if (!(manKey in newRegister.manufacturers)) {
        newRegister.manufacturers[manKey] = (manKey in register.manufacturers) ? register.manufacturers[manKey] : [];
      }
      ...
      for (const cat of out.fixtures[fixtureKey].categories) {
        if (!(cat in newRegister.categories)) {
          newRegister.categories[cat] = (cat in register.categories) ? register.categories[cat] : [];
        }
  ...
  .then(() => {
    return addOrUpdateFile('fixtures/register.json', 'register.json', oldFileContent => {
      ...
      const oldRegister = JSON.parse(oldFileContent);
      for (const reg in register) {
        newRegister[reg] = Object.assign({}, oldRegister[reg], newRegister[reg]);
      }
      return prettyStringify(newRegister);
    });
  })
