...
module.exports = function(args) {
  ...
  child_process.exec(
    ...
    (error, stdout, stderr) => {
      ...
      // make recursive diff
      diffDir('', compareOut, currentOut);
      findAddedFiles('', compareOut, currentOut);
      console.log('Done.');
    }
  );
}
...
function diffDir(subdir, compareOut, currentOut) {
  ...
  for (const file of files) {
    ...
    if (!fs.existsSync(filePathCurrent)) {
      ...
      if (fs.lstatSync(filePathCompare).isDirectory()) {
        diffDir(filePath, compareOut, currentOut);
      }
      else {
...
function findAddedFiles(subdir, compareOut, currentOut) {
  ...
  for (const file of files) {
    ...
    if (!fs.existsSync(filePathCompare)) {
      ...
      findAddedFiles(filePath, compareOut, currentOut);
    }
  }
}
