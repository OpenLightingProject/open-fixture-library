...
module.exports = async function diffPluginOutputs(currentPluginKey, comparePluginKey, ref, fixtures) {
  ...
  console.log(`# ref:`, ref);
  console.log(`# fixtures:`, fixtures.map(([manufacturer, fix]) => `${manufacturer}/${fix}`).join(`, `));
  console.log();

  const temporaryDirectory = path.join(__dirname, `..`, `tmp`);
  if (process.cwd().match(temporaryDirectory)) {
    console.error(`${chalk.red(`[Error]`)} This script can't be run from inside the tmp directory.`);
    process.exit(1);
  }

  const currentOut = path.join(temporaryDirectory, `current_output`);
  const compareOut = path.join(temporaryDirectory, `compare_output`);
  ...
  const compareArchivePath = path.join(temporaryDirectory, `compare.zip`);
  const unzipDirectory = path.join(temporaryDirectory, `compareFiles`);
  ...
  await Promise.all(Object.keys(zip.files).map(async fileName => {
    ...
    const filePath = path.join(unzipDirectory, file.name);
  ...
  try {
    await exportFixtures(
      compareOut,
      path.join(unzipDirectory, `plugins`, comparePluginKey, `export.js`),
      unzipDirectory,
    );
  }
  catch (error) {
   ...
   * @param {String} baseDirectory The OFL root directory.
   * @returns {Promise.<*, Error>} A Promise that resolves when all exported fixtures are saved to the filesystem.
   */
  async function exportFixtures(outputPath, pluginScript, baseDirectory) {
    const plugin = require(pluginScript);
    const { fixtureFromRepository } = require(path.join(baseDirectory, `lib`, `model.js`));

    const outFiles = await plugin.export(fixtures.map(([manufacturer, fix]) => fixtureFromRepository(manufacturer, fix)), {
      baseDirectory,
      date,
