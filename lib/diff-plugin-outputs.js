...
module.exports = function(plugin, ref, fixtures) {
  ...
  for (const difference of differences) {
    ...
    switch (difference.state) {
      ...
      case 'distinct':
        name = getRelativePath(difference.relativePath, difference.name2, difference.type2);
        const patch = disparity.unified(
          fs.readFileSync(path.join(difference.path1, difference.name1), 'utf8'),
          fs.readFileSync(path.join(difference.path2, difference.name2), 'utf8')
        );

        console.log(colors.yellow(`Diff for ${name}`));
        console.log(patch);

        outputData.changedFiles[name] = patch;
        continue;
    }
  }
