...
export default async function diffPluginOutputs(currentPluginKey, comparePluginKey, ref, fixtures) {
  ...
  /**
   *
   * @param {Readonly<URL>} outputUrl - The path where to output the exported fixtures.
   * @param {string} pluginKey - The plugin key.
   * @param {Readonly<URL>} baseDirectory - The OFL root directory.
...
async function findDifferences(currentOut, compareOut) {
  ...
  for (const difference of diffResult.diffSet) {
    await addDifference(outputData, difference);
  }
...
/**
 * Records a single file/directory difference in the output data.
 * @param {PluginDiffOutput} outputData - The diff output data to add the difference to.
 * @param {object} difference - A single entry from the directory comparison's diff set.
 */
async function addDifference(outputData, difference) {
  switch (difference.state) {
    case 'left': {
      const name = getRelativePath(difference.relativePath, difference.name1, difference.type1);
    ...
    case 'right': {
      const name = getRelativePath(difference.relativePath, difference.name2, difference.type2);
