...
module.exports = function(args, resolve) {
  ...
  const fixPaths = args.fixtures.map(relativePath => {
    const absolutePath = path.join(process.env.PWD, relativePath);
    return path.join(
      'fixtures',
      path.basename(path.dirname(absolutePath)), // man dir
      path.basename(absolutePath) // fix file
    );
  });
  ...
  console.log('# fixtures:', fixPaths.join(', '));
  console.log();
  ...
  function exportFixtures(outputPath, pluginScript, baseDir) {
    ...
    const outFiles = plugin.export(
      fixPaths.map(
        fixPath => new Fixture(require(path.join(baseDir, fixPath)))
      ),
      {
        baseDir: baseDir
      }
