...
module.exports = async function diffPluginOutputs(currentPluginKey, comparePluginKey, ref, fixtures) {
  ...
  if (process.cwd().startsWith(temporaryDirectory)) {
  ...
  try {
    await exportFixtures(currentOut, currentPluginKey, path.join(__dirname, `..`));
  }
  catch (error) {
  ...
  try {
    await exportFixtures(compareOut, comparePluginKey, unzipDirectory);
  }
  catch (error) {
   ...
   */
  async function exportFixtures(outputPath, pluginKey, baseDirectory) {
    const plugin = require(path.join(baseDirectory, `plugins/${pluginKey}/export.js`));
