...
async function getResourceFromString(resourceName) {
  ...
  try {
    ...
    throw error instanceof SyntaxError
      ? new Error(`Resource file '${resourceUrl.pathname}' could not be parsed as JSON.`)
...
async function getImageForResource(type, baseUrl, key) {
  ...
  if (type === `gobos`) {
    ...
    throw new Error(`Expected gobo image for resource '${new URL(key, baseUrl).pathname}' not found (supported file extensions: ${fileExtensions.join(`, `)}).`);
  }
