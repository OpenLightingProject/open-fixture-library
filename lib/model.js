...
export async function fixtureFromFile(absolutePath) {
  ...
  if (fixtureJson.$schema.endsWith('/fixture-redirect.json')) {
    [manufacturerKey, fixtureKey] = fixtureJson.redirectTo.split('/', 2);
  ...
  try {
    ...
    throw new Error(
      error instanceof SyntaxError
        ? `Resource file '${fileURLToPath(resourceUrl)}' could not be parsed as JSON.`
 ...
 * @param {Readonly<URL>} baseUrl - The path of the resource directory.
