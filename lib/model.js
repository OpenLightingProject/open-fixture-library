...
function fixtureFromFile(absolutePath) {
  ...
  fixJson = embedResourcesIntoFixtureJson(fixJson);
...
/**
 * @param {String} fixJson The fixture JSON.
 * @returns {String} The same fixture JSON, but with resources embedded.
 */
...
function getResourceFromString(resourceName) {
  ...
  const image = getImageForResource(type, resourceBasePath, key);
  if (image) {
    resourceData.image = image.dataUrl;
    resourceData.imageExtension = image.extension;
  }
 ...
 * @property {String} extension The extions of the image file.
 */
...
function getImageForResource(type, basePath, key) {
  ...
  for (const extension of extensions) {
    try {
      const base64 = (extension !== `svg`);
      let imageData = fs.readFileSync(`${basePath}/${key}.${extension}`, base64 ? `base64` : `utf8`);
      ...
      const dataUrl = `data:${mimeType},${imageData}`;
