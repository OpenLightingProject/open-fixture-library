...
function hasFixtureInfinitePanTilt(fixture, panOrTilt) {
  ...
  return physicals.some(hasPhysicalInfinitePanTilt);

  /**
   * @param {!Physical} physical A fixture's or mode's physical data.
   * @returns {!boolean} Whether the given physical data specifies endless pan/tilt.
   */
  function hasPhysicalInfinitePanTilt(physical) {
    return physical && physical[`focus${panOrTilt}Max`] === Number.POSITIVE_INFINITY;
  }
}
