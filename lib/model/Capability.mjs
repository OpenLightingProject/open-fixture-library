...
const namePerType = {
  ...
  GoboShake: cap => {
    ...
    if (cap.speed) {
      str += ` ${startEndToString(cap.speed)}`;
    }

    if (cap.swingAngle) {
      str += ` ${startEndToString(cap.swingAngle)}`;
    }
...
export default class Capability {
  ...
  /**
   * @returns {?Array.<Entity>} Start and end swing angle values. Defaults to null.
   */
  get swingAngle() {
    if (!(`swingAngle` in this._cache)) {
      this._cache.swingAngle = this._getStartEndArray(`swingAngle`);
    }

    return this._cache.swingAngle;
  }
