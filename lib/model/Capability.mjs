...
const namePerType = {
  ...
  GoboIndex: cap => {
    ...
    if (cap.hasComment) {
      name = cap.comment.replace(/^(?:gobo\s+|)(?!open|closed)/i, `Gobo `);
    }
    else {
      name = `Gobo ${startEndToString(cap.index)}`;
    }
...
class Capability {
  ...
  get colors() {
    if (!(`colors` in this._cache)) {
      const fixedColors = {
        Open: [`#ffffff`],
        Closed: [`#000000`],
        'Open … Closed': [`#ffffff`, `#000000`],
        'Closed … Open': [`#000000`, `#ffffff`]
      };
      ...
      else if (this.type === `GoboIndex` && Object.keys(fixedColors).includes(this.comment)) {
        this._cache.colors = {
          startColors: fixedColors[this.comment],
          endColors: fixedColors[this.comment],
          allColors: fixedColors[this.comment],
          isStep: true
        };
      }
      else {
  ...
  /**
   * @returns {array.<Entity>|null} Start and end index values. Defaults to null.
   */
  get index() {
