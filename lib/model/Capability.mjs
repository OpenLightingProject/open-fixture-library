...
const START_END_PROPERTIES = [`speed`, `duration`, `time`, `brightness`, `index`, `angle`, `colorTemperature`, `effectIntensity`, `soundSensitivity`, `shakeAngle`, `shakeSpeed`, `distance`, `openPercent`, `frostIntensity`, `insertion`, `fogOutput`, `parameter`];
...
const namePerType = {
  ...
  FogOff: cap => {
    return appendInBrackets(`Fog off`, cap.comment);
  },
  FogOn: cap => {
  ...
  get isInverted() {
    if (!(`isInverted` in this._cache)) {
      if (this.isStep) {
        ...
        const proportionalProps = Capability.START_END_PROPERTIES.filter(
