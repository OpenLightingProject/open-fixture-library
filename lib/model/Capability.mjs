...
const namePerType = {
  ...
  WheelShake: cap => {
    let name = cap.wheels.map(wheel => wheel.name).join(`, `);

    if (cap.slotNumber) {
      name = getSlotCapabilityName(cap);
    }
  ...
  WheelSlotRotation: cap => {
    let wheelSlotType = cap.wheelSlot ? cap.wheelSlot[0].type : cap.wheels[0].type;
    if (wheelSlotType === `Gobo`) {
      wheelSlotType = `Gobo stencil`;
    }

    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`${wheelSlotType} rotation ${startEndToString(cap[speedOrAngle], speedOrAngle, true)}`, cap.comment);
  },
  WheelRotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`${cap.wheels[0].name} rotation ${startEndToString(cap[speedOrAngle], speedOrAngle, true)}`, cap.comment);
  },
  Effect: cap => {
...
class Capability {
  ...
  get wheels() {
    if (!(`wheels` in this._cache)) {
      ...
      if (`wheel` in this._jsonObject) {
        wheelNames = Array.isArray(this._jsonObject.wheel) ? this._jsonObject.wheel : [this._jsonObject.wheel];
      }
      ...
      this._cache.wheels = wheelNames.map(wheelName => this._channel.fixture.wheels.find(
  ...
  /**
   * @returns {object} Start and end wheel slot objects this capability is referencing. Defaults to null.
   */
  get wheelSlot() {
    ...
    if (!(`wheelSlot` in this._cache)) {
      this._cache.wheelSlot = this.slotNumber.map(slotNumber => this.wheels[0].getSlot(slotNumber.number));
    }
