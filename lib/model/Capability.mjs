...
const namePerType = {
  ...
  'ShutterStrobe': cap => {
    ...
    if (cap.speed) {
      name += ` ${startEndToString(cap, `speed`)}`;
    }

    if (cap.duration) {
      name += ` ${startEndToString(cap, `duration`)}`;
    }
...
/**
 * @param {!Capability} cap The capability whose property should be accessed.
 * @param {!string} propertyName The property name in the model, e.g. 'speed'.
 * @returns {!string} A self-explaining string describing the property value (including start-end information, if given).
 */
function startEndToString(cap, propertyName) {
  const [start, end] = cap[propertyName];
  let str;
  ...
  if (!start.equals(end)) {
    str = `${start.number}…${end.number}${start.unit}`;
  }
  else {
    str = `${start.number}${start.unit}`;
  }
