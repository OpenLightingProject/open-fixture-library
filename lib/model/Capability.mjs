...
export default class Capability {
  ...
  /**
   * @param {!number} resolution The grade of resolution the dmxRange should be scaled to.
   * @returns {!Range} The capability's DMX bounds scaled (down) to the given resolution.
   */
  getDmxRangeWithResolution(resolution) {
    const max = this._channel.maxResolution;
    if (resolution > max || resolution < Channel.RESOLUTION_8BIT || resolution % 1 !== 0) {
      throw new RangeError(`resolution must be a positive integer not greater than channel ${this._channel.key}'s maxResolution`);
    }

    if (!this._cache.dmxRangePerResolution[resolution]) {
      this._cache.dmxRangePerResolution[resolution] = new Range([
        Math.floor(this._jsonObject.dmxRange[0] * Math.pow(256, resolution - this._resolution)),
        Math.floor(((this._jsonObject.dmxRange[1] + 1) * Math.pow(256, resolution - this._resolution)) - 1)
      ]);
    }

    return this._cache.dmxRangePerResolution[resolution];
  }
  ...
  get hold() {
    if (!(`hold` in this._cache)) {
      this._cache.hold = `hold` in this._jsonObject ? new Entity(this._jsonObject.hold) : null;
    }
  ...
  get brightness() {
    if (!(`brightness` in this._cache)) {
      ...
      if (brightness === null && [`Intensity`, `ColorIntensity`].includes(this.type)) {
        brightness = [new Entity(`off`), new Entity(`bright`)];
      }
