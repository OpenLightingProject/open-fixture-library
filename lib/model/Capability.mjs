...
const namePerType = {
  ...
  Pan: cap => {
    return appendInBrackets(`Pan ${startEndToString(cap.angle, `angle`, true)}`, cap.comment);
  },
  PanContinuous: cap => {
    return appendInBrackets(`Pan ${startEndToString(cap.speed, `speed`, true)}`, cap.comment);
  },
  Tilt: cap => {
    return appendInBrackets(`Tilt ${startEndToString(cap.angle, `angle`, true)}`, cap.comment);
  },
  TiltContinuous: cap => {
    return appendInBrackets(`Tilt ${startEndToString(cap.speed, `speed`, true)}`, cap.comment);
  },
  PanTiltSpeed: cap => {
  ...
  BladeRotation: cap => {
    return appendInBrackets(`Blade ${cap.blade} rotation ${startEndToString(cap.angle, `angle`, true)}`, cap.comment);
  },
  BladeSystemRotation: cap => {
    return appendInBrackets(`Blade system rotation ${startEndToString(cap.angle, `angle`, true)}`, cap.comment);
  },
  Fog: cap => {
  ...
  BeamAngle: cap => {
    return appendInBrackets(`Beam ${startEndToString(cap.angle, `angle`, true)}`, cap.comment);
  },
  Rotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
