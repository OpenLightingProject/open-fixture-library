...
const START_END_PROPERTIES = [`speed`, `duration`, `brightness`, `index`, `angle`, `colorTemperature`, `effectIntensity`, `soundSensitivity`, `shakeAngle`, `shakeSpeed`, `distance`, `openPercent`, `frostIntensity`, `insertion`, `fogOutput`, `parameter`];
...
const namePerType = {
  ...
  GoboIndex: cap => {
    ...
    if (cap.isShaking) {
      ...
      if (cap.shakeAngle) {
        str += ` ${startEndToString(cap.shakeAngle)}`;
      }

      if (cap.shakeSpeed) {
        str += ` ${startEndToString(cap.shakeSpeed)}`;
      }
    }
...
export default class Capability {
  ...
  get isShaking() {
    if (!(`isShaking` in this._cache)) {
      this._cache.isShaking = this.shakeAngle !== null && (this.shakeAngle[0].number > 0 || this.shakeAngle[1].number > 0);
    }
