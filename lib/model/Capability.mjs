...
const START_END_ENTITIES = [`speed`, `duration`, `time`, `brightness`, `index`, `angle`, `colorTemperature`, `effectIntensity`, `soundSensitivity`, `shakeAngle`, `shakeSpeed`, `distance`, `openPercent`, `frostIntensity`, `insertion`, `fogOutput`, `parameter`];
...
const namePerType = {
  ...
  ShutterStrobe: cap => {
    let name = {
      ...
      RampUpDownRandom: `Random ramp up and down strobe`,
  ...
  ColorWheelRotation: cap => {
    ...
    return appendInBrackets(`Color wheel rotation ${startEndToString(cap[speedOrAngle], speedOrAngle)}`, cap.comment);
  },
  ColorTemperature: cap => {
    return appendInBrackets(`Color temperature ${colorTemperaturesToString(cap.colorTemperature)}`, cap.comment);
  },
  Pan: cap => {
    return appendInBrackets(`Pan ${startEndToString(cap.angle, `angle`)}`, cap.comment);
  },
  PanContinuous: cap => {
    return appendInBrackets(`Pan ${startEndToString(cap.speed, `speed`)}`, cap.comment);
  },
  Tilt: cap => {
    return appendInBrackets(`Tilt ${startEndToString(cap.angle, `angle`)}`, cap.comment);
  },
  TiltContinuous: cap => {
    return appendInBrackets(`Tilt ${startEndToString(cap.speed, `speed`)}`, cap.comment);
  },
  PanTiltSpeed: cap => {
  ...
  SoundSensitivity: cap => {
    return appendInBrackets(`Sound sensitivity ${startEndToString(cap.soundSensitivity)}`, cap.comment);
  },
  GoboIndex: cap => {
    let name = `Gobo `;
  ...
  GoboShake: cap => {
    ...
    if (cap.shakeAngle) {
      name += ` ${startEndToString(cap.shakeAngle, `angle`)}`;
    }
  ...
  GoboStencilRotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`Gobo stencil rotation ${startEndToString(cap[speedOrAngle], speedOrAngle)}`, cap.comment);
  },
  GoboWheelRotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`Gobo wheel rotation ${startEndToString(cap[speedOrAngle], speedOrAngle)}`, cap.comment);
  },
  Focus: cap => {
  ...
  PrismRotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`Prism rotation ${startEndToString(cap[speedOrAngle], speedOrAngle)}`, cap.comment);
  },
  BladeInsertion: cap => {
    return appendInBrackets(`Blade ${cap.blade} insertion ${startEndToString(cap.insertion)}`, cap.comment);
  },
  BladeRotation: cap => {
    return appendInBrackets(`Blade ${cap.blade} rotation ${startEndToString(cap.angle, `angle`)}`, cap.comment);
  },
  BladeSystemRotation: cap => {
    return appendInBrackets(`Blade system rotation ${startEndToString(cap.angle, `angle`)}`, cap.comment);
  },
  Fog: cap => {
  ...
  FogOutput: cap => {
    return appendInBrackets(`Fog output ${startEndToString(cap.fogOutput)}`, cap.comment);
  },
  FogType: cap => {
    return appendInBrackets(`Fog type: ${cap.fogType}`, cap.comment);
  },
  BeamAngle: cap => {
    return appendInBrackets(`Beam ${startEndToString(cap.angle, `angle`)}`, cap.comment);
  },
  Rotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
    return appendInBrackets(`Rotation ${startEndToString(cap[speedOrAngle], speedOrAngle)}`, cap.comment);
  },
  Speed: cap => {
    return appendInBrackets(`Speed ${startEndToString(cap.speed)}`, cap.comment);
  },
  Time: cap => {
    return appendInBrackets(`Time ${startEndToString(cap.time)}`, cap.comment);
  },
  Maintenance: cap => {
...
export default class Capability {
  ...
  /**
   * @param {!Capability} capability The next capability after this one.
   ...
   */
  canCrossfadeTo(capability) {
    if (this.type !== capability.type) {
      return false;
    }

    if (this.usedStartEndEntities.length === 0 || this.usedStartEndEntities.length !== capability.usedStartEndEntities.length) {
      return false;
    }

    const usesSameStartEndEntities = this.usedStartEndEntities.every(
      prop => capability.usedStartEndEntities.includes(prop)
    );
    ...
    return this.usedStartEndEntities.every(prop => {
      ...
      const delta = capability[prop][0].number - this[prop][1].number;
