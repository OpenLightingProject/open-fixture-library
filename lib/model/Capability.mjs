...
import { scaleDmxRange } from '../scale-dmx-values.mjs';
...
const namePerType = {
  ...
  BeamPosition: cap => {
    if (cap.horizontalAngle) {
      if (cap.verticalAngle) {
      ...
      const prefix = cap.horizontalAngle[0].keyword === null ? `Horizontal beam position` : `Beam position`;
      return appendInBrackets(`${prefix} ${startEndToString(cap.horizontalAngle)}`, cap.comment);
    }

    const prefix = cap.verticalAngle[0].keyword === null ? `Vertical beam position` : `Beam position`;
    return appendInBrackets(`${prefix} ${startEndToString(cap.verticalAngle)}`, cap.comment);
  },
  Rotation: cap => {
    const speedOrAngle = cap.speed !== null ? `speed` : `angle`;
