...
const namePerType = {
  ...
  ShutterStrobe: cap => {
    ...
    return appendComment(name, cap.comment);
  },
  StrobeSpeed: cap =>
    appendComment(`Strobe speed ${startEndToString(cap.speed)}`, cap.comment),
  StrobeDuration: cap =>
    appendComment(`Strobe duration ${startEndToString(cap.duration)}`, cap.comment),
  Intensity: cap =>
    appendComment(`Intensity ${startEndToString(cap.brightness)}`, cap.comment),
  ColorIntensity: cap =>
    appendComment(`${cap.color} ${startEndToString(cap.brightness)}`, cap.comment),
  ColorPreset: cap => {
  ...
  ColorWheelRotationAngle: cap =>
    appendComment(`Color wheel rotation ${startEndToString(cap.angle, `angle`)}`, cap.comment),
  ColorWheelRotationSpeed: cap =>
    appendComment(`Color wheel rotation ${startEndToString(cap.speed, `speed`)}`, cap.comment),
  Pan: cap =>
    appendComment(`Pan ${startEndToString(cap.angle, `angle`)}`, cap.comment),
  PanContinuous: cap =>
    appendComment(`Continuous pan ${startEndToString(cap.speed, `speed`)}`, cap.comment),
  Tilt: cap =>
    appendComment(`Tilt ${startEndToString(cap.angle, `angle`)}`, cap.comment),
  TiltContinuous: cap =>
    appendComment(`Continuous tilt ${startEndToString(cap.speed, `speed`)}`, cap.comment),
  PanTiltSpeed: cap => {
  ...
  Effect: cap => {
    ...
    if (cap.soundSensitivity) {
      name += ` ${startEndToString(cap.soundSensitivity)} sound sensitivity`;
    }

    return appendComment(name, cap.comment);
  },
  EffectSpeed: cap =>
    appendComment(`Effect speed ${startEndToString(cap.speed)}`, cap.comment),
  EffectDuration: cap =>
    appendComment(`Effect duration ${startEndToString(cap.duration)}`, cap.comment),
  EffectIntensity: cap =>
    appendComment(`Effect intensity ${startEndToString(cap.effectIntensity)}`, cap.comment),
  SoundSensitivity: cap =>
    appendComment(`Sound sensitivity ${startEndToString(cap.sensitivity)}`, cap.comment)
};
...
/**
 * @param {?string} str The original string without comment.
 * @param {?string} comment The comment that may be null or undefined.
 * @returns {!string} The given string, appended with a space and the given comment if the comment was valid.
 */
function appendComment(str, comment) {
  if (comment) {
    return `${str} (${comment})`;
  }
