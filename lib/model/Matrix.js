...
class Matrix {
  /**
   * @param {Readonly<object>} jsonObject - The fixture's JSON object containing the matrix information.
   */
  ...
  getPixelKeysByOrder(firstAxis, secondAxis, thirdAxis) {
    ...
    return this.pixelKeys.toSorted((keyA, keyB) => {
      const posA = this.pixelKeyPositions[keyA];
      const posB = this.pixelKeyPositions[keyB];

      const isThirdPosDifferent = posA[thirdPosIndex] !== posB[thirdPosIndex];
      if (isThirdPosDifferent) {
        return posA[thirdPosIndex] - posB[thirdPosIndex];
      }

      const isSecondPosDifferent = posA[secondPosIndex] !== posB[secondPosIndex];
      return isSecondPosDifferent
        ? posA[secondPosIndex] - posB[secondPosIndex]
        : posA[firstPosIndex] - posB[firstPosIndex];
    });
  }
  ...
  _pixelKeyFulfillsConstraints(pixelKey, constraints) {
    ...
    const areNumberConstraintsFulfilled = ['x', 'y', 'z'].every((axis, axisIndex) => {
      const axisPos = position[axisIndex];

      return constraints[axis].every((constraintFunction) => constraintFunction(axisPos));
    });

    const areStringConstraintsFulfilled = constraints.name.every(
      (constraintFunction) => constraintFunction(pixelKey),
    );

    return areNumberConstraintsFulfilled && areStringConstraintsFulfilled;
  }
}
