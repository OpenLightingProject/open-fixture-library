...
module.exports = class Matrix {
  ...
  get pixelCount() {
    if (!('pixelCount' in this._cache)) {
      if ('pixelCount' in this._jsonObject) {
        ...
        let xyz = [0, 0, this.pixelKeys.length];
  ...
  get pixelKeys() {
    if (!('pixelKeys' in this._cache)) {
      if ('pixelKeys' in this._jsonObject) {
        ...
        for (let z = 1; z <= this.zPixelCount; z++) {
          let yItems = [];
          
          for (let y = 1; y <= this.yPixelCount; y++) {
            let xItems = [];
            
            for (let x = 1; x <= this.xPixelCount; x++) {
  ...
  _getPixelDefaultKey(x, y, z) {
    switch (this.layout) {
      ...
      case 'rect':
        const first = this.xPixelCount > 1 ? x : y;
        const last = this.yPixelCount > 1 ? y : z;
        return `(${first}, ${last})`;
