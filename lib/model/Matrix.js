      ...
      [`x`, `y`, `z`].forEach(axis => {
        constraintFunctions[axis] = (constraints[axis] || []).map(constraint => {
          if (constraint.startsWith(`=`)) {
            const eqPos = parseInt(constraint.slice(1), 10);
            return (position => position === eqPos);
          }

          if (constraint.startsWith(`>=`)) {
            const minPos = parseInt(constraint.slice(2), 10);
            return (position => position >= minPos);
          }

          if (constraint.startsWith(`<=`)) {
            const maxPos = parseInt(constraint.slice(2), 10);
          ...
          if (constraint.match(/^(\d+)n(?:\+(\d+)|)$/)) {
            const divisor = parseInt(RegExp.$1, 10);
            const remainder = parseInt(RegExp.$2 || `0`, 10);
