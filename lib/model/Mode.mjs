...
export default class Mode {
    ...
    if (!(`channelKeys` in this._cache)) {
      ...
      for (const rawReference of this._jsonObject.channels) {
        if (rawReference !== null && rawReference.insert === `matrixChannels`) {
          ...
          chKeys = chKeys.concat(this._getMatrixChannelKeysByInsertBlock(rawReference));
        }
        else {
          // normal channel key
   ...
   */
  _getMatrixChannelKeysByInsertBlock(channelInsert) {
    const pixelKeys = this._getPixelKeysByRepeatFor(channelInsert.repeatFor);
   ...
   */
  _getPixelKeysByRepeatFor(repeatFor) {
