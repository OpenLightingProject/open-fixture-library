...
export default class Channel extends AbstractChannel {
    ...
    if (!(`dmxValueResolution` in this._cache)) {
      if (`dmxValueResolution` in this._jsonObject) {
        const resolutionStringToResolution = {
          '8bit': Channel.RESOLUTION_8BIT,
          '16bit': Channel.RESOLUTION_16BIT,
          '24bit': Channel.RESOLUTION_24BIT
        };
  ...
  /**
   * @param {!Resolution} resolution The grade of resolution the defaultValue should be scaled to.
   * @returns {!number} The DMX value this channel initially should be set to, scaled to match the given resolution.
   */
  getDefaultValueWithResolution(resolution) {
    if (resolution > this.maxResolution || resolution < Channel.RESOLUTION_8BIT || resolution % 1 !== 0) {
      throw new RangeError(`resolution must be a positive integer not greater than maxResolution`);
    }

    if (!(resolution in this._cache.defaultValuePerResolution)) {
      this._cache.defaultValuePerResolution[resolution] = Math.floor((this._jsonObject.defaultValue || 0) * Math.pow(256, resolution - this.dmxValueResolution));
    }

    return this._cache.defaultValuePerResolution[resolution];
  }
  ...
  /**
   * @param {!Resolution} resolution The grade of resolution the highlightValue should be scaled to.
   * @returns {!number} A DMX value that "highlights" the function of this channel, scaled to match the given resolution.
   */
  getHighlightValueWithResolution(resolution) {
    if (resolution > this.maxResolution || resolution < Channel.RESOLUTION_8BIT || resolution % 1 !== 0) {
      throw new RangeError(`resolution must be a non-negative integer not greater than maxResolution`);
    }

    if (!(resolution in this._cache.highlightValuePerResolution)) {
      let highlightValue;
      if (this.hasHighlightValue) {
        highlightValue = Math.floor(this._jsonObject.highlightValue * Math.pow(256, resolution - this.dmxValueResolution));
      }
      else {
        highlightValue = Math.floor(this.maxDmxBound * Math.pow(256, resolution - this.maxResolution));
      }

      this._cache.highlightValuePerResolution[resolution] = highlightValue;
    }

    return this._cache.highlightValuePerResolution[resolution];
  }
