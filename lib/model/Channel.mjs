...
import ScaleDmxValues from '../scale-dmx-values.js';
const { scaleDmxValue } = ScaleDmxValues;
...
export default class Channel extends AbstractChannel {
  ...
  getDefaultValueWithResolution(desiredResolution) {
    if (desiredResolution > this.maxResolution || desiredResolution < Channel.RESOLUTION_8BIT || desiredResolution % 1 !== 0) {
      throw new RangeError(`resolution must be a positive integer not greater than maxResolution`);
    }

    if (!(desiredResolution in this._cache.defaultValuePerResolution)) {
      this._cache.defaultValuePerResolution[desiredResolution] = scaleDmxValue(
        this._jsonObject.defaultValue || 0,
        this.dmxValueResolution,
        desiredResolution
      );
    }
  ...
  getHighlightValueWithResolution(desiredResolution) {
    ...
    if (!(desiredResolution in this._cache.highlightValuePerResolution)) {
      let highlightValue;
      if (this.hasHighlightValue) {
        highlightValue = scaleDmxValue(this._jsonObject.highlightValue, this.dmxValueResolution, desiredResolution);
      }
      else {
        highlightValue = scaleDmxValue(this.maxDmxBound, this.maxResolution, desiredResolution);
      }

      this._cache.highlightValuePerResolution[desiredResolution] = highlightValue;
    }
