...
export default class Channel extends AbstractChannel {
  ...
  get dmxValueResolution() {
    return `dmxValueResolution` in this._jsonObject ? this._jsonObject.dmxValueResolution : this.maxFineness;
  }
  ...
  get capabilities() {
    if (!(`capabilities` in this._cache)) {
      if (`capability` in this._jsonObject) {
        const capabilityData = Object.assign({ dmxRange: [0, Math.pow(256, this.dmxValueResolution + 1) - 1] }, this._jsonObject.capability);
