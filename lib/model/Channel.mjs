...
export default class Channel extends AbstractChannel {
  ...
  /**
   * @returns {!Fineness} How fine this channel can be used at its maximum. Equals the amout of fine channels.
   */
  get maxFineness() {
   ...
   */
  get definitionFineness() {
    return `definitionFineness` in this._jsonObject ? this._jsonObject.definitionFineness : this.maxFineness;
  }
   ...
   */
  get maxDmxBound() {
    return Math.pow(256, this.maxFineness + 1) - 1;
  }
  ...
  getDefaultValueWithFineness(fineness) {
    ...
    if (!(fineness in this._cache.defaultValuePerFineness)) {
      this._cache.defaultValuePerFineness[fineness] = Math.floor((this._jsonObject.defaultValue || 0) * Math.pow(256, fineness - this.definitionFineness));
    }
  ...
  getHighlightValueWithFineness(fineness) {
    ...
    if (!(fineness in this._cache.highlightValuePerFineness)) {
      ...
      if (this.hasHighlightValue) {
        highlightValue = Math.floor(this._jsonObject.highlightValue * Math.pow(256, fineness - this.definitionFineness));
      }
      else {
  ...
  get capabilities() {
    if (!(`capabilities` in this._cache)) {
      if (`capability` in this._jsonObject) {
        const capabilityData = Object.assign({ dmxRange: [0, Math.pow(256, this.definitionFineness + 1) - 1] }, this._jsonObject.capability);

        this._cache.capabilities = [
          new Capability(capabilityData, this.definitionFineness, this)
        ];
      }
      else {
        this._cache.capabilities = this._jsonObject.capabilities.map(
          cap => new Capability(cap, this.definitionFineness, this)
        );
      }
    }
