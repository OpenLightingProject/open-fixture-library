...
module.exports = class Fixture {
  ...
  get matrixChannelReferences() {
    if (!('matrixChannelReferences' in this._cache)) {
      ...
      for (const mode of this.modes) {
        for (const templateChannelKey of Object.keys(mode.matrixChannelReferences)) {
          if (!(templateChannelKey in matrixChannelReferences)) {
            matrixChannelReferences[templateChannelKey] = mode.matrixChannelReferences[templateChannelKey];
          }
          else {
            matrixChannelReferences[templateChannelKey] = new Set([
              ...matrixChannelReferences[templateChannelKey],
              ...mode.matrixChannelReferences[templateChannelKey]
            ]);
          }
        }
      }
  ...
  /**
   * @returns {!Array.<string>} All used matrix channels' keys.
   */
  ...
  /**
   * @returns {!Array.<MatrixChannel>} All used matrix channels.
   */
  get matrixChannels() {
    if (!('matrixChannels' in this._cache)) {
      let channels = [];

      for (const templateChannelKey of Object.keys(this.matrixChannelReferences)) {
        ...
        if (templateChannel !== null) {
          for (const pixelKey of this.matrixChannelReferences[templateChannelKey]) {
  ...
  get allChannels() {
    if (!('allChannels' in this._cache)) {
      ...
      // first, add availableChannels (without channels that override matrix channels, as we include them in matrixChannels as well)
      for (const channel of this.availableChannels.filter(ch => !this.matrixChannelKeys.includes(ch.key))) {
