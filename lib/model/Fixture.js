...
class Fixture {
   ...
   * @param {String|Manufacturer} manufacturer Either the fixture's manufacturer's key or a Manufacturer instance.
  ...
  constructor(manufacturer, key, jsonObject) {
    this.manufacturer = manufacturer; // calls the setter
    this._key = key;
  ...
  /**
   * @param {String|Manufacturer} newManufacturer Either the fixture's manufacturer's key or a Manufacturer instance.
   */
  set manufacturer(newManufacturer) {
    if (newManufacturer instanceof Manufacturer) {
      this._manufacturer = newManufacturer;
    }
    else {
      this._manufacturer = new Manufacturer(newManufacturer);
    }
  }
  ...
  get isCapabilityHelpWanted() {
    if (!(`isCapabilityHelpWanted` in this._cache)) {
      this._cache.isCapabilityHelpWanted = this.allChannels.some(channel => channel.isHelpWanted);
    }
  ...
  get uniqueChannelNames() {
    if (!(`uniqueChannelNames` in this._cache)) {
      ...
      for (let index = 0; index < names.length; index++) {
        ...
        // save unique name
        this._cache.uniqueChannelNames[this.allChannelKeys[index]] = names[index];
      }
    }
  ...
  get coarseChannelKeys() {
    if (!(`coarseChannelKeys` in this._cache)) {
      this._cache.coarseChannelKeys = this.coarseChannels.map(channel => channel.key);
    }
  ...
  get coarseChannels() {
    if (!(`coarseChannels` in this._cache)) {
      this._cache.coarseChannels = this.allChannels.filter(channel => channel instanceof CoarseChannel);
    }
  ...
  get fineChannelAliases() {
    if (!(`fineChannelAliases` in this._cache)) {
      this._cache.fineChannelAliases = this.fineChannels.map(channel => channel.key);
    }
  ...
  get fineChannels() {
    if (!(`fineChannels` in this._cache)) {
      this._cache.fineChannels = this.allChannels.filter(channel => channel instanceof FineChannel);
    }
  ...
  get switchingChannelAliases() {
    if (!(`switchingChannelAliases` in this._cache)) {
      this._cache.switchingChannelAliases = this.switchingChannels.map(channel => channel.key);
    }
  ...
  get switchingChannels() {
    if (!(`switchingChannels` in this._cache)) {
      this._cache.switchingChannels = this.allChannels.filter(channel => channel instanceof SwitchingChannel);
    }
  ...
  get matrixChannelKeys() {
    if (!(`matrixChannelKeys` in this._cache)) {
      this._cache.matrixChannelKeys = this.matrixChannels.map(channel => channel.key);
    }
  ...
  get matrixChannels() {
    ...
    if (!(`matrixChannels` in this._cache)) {
      this._cache.matrixChannels = this.allChannels.filter(channel => channel.pixelKey !== null);
    }
  ...
  get nullChannels() {
    if (!(`nullChannels` in this._cache)) {
      ...
      for (let index = 0; index < maxNullPerMode; index++) {
  ...
  get allChannelsByKey() {
    if (!(`allChannelsByKey` in this._cache)) {
      ...
      this.availableChannels.forEach(mainChannel => {
        [mainChannel, ...mainChannel.fineChannels, ...mainChannel.switchingChannels].forEach(channel => {
          allChannelsByKey[channel.key] = channel;
        });
      });

      this.nullChannels.forEach(channel => {
        allChannelsByKey[channel.key] = channel;
      });

      const allMatrixChannelsByKey = {};
      this.templateChannels.forEach(templateChannel => {
        templateChannel.createMatrixChannels().forEach(channel => {
