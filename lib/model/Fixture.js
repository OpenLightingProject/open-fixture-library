...
class Fixture {
   ...
   * @param {Readonly<Manufacturer>} manufacturer - A Manufacturer instance.
   * @param {string} key - The fixture's unique key. Equals to filename without '.json'.
   * @param {Readonly<object>} jsonObject - The fixture's parsed JSON data.
   */
   ...
   */
  getLinksOfType(type) {
    return this.links === null ? [] : this.links[type] || [];
  }
   ...
   */
  get physical() {
    const physical = 'physical' in this._jsonObject ? new Physical(this._jsonObject.physical) : null;

    return cacheResult(this, 'physical', physical);
  }
   ...
   */
  get matrix() {
    const matrix = 'matrix' in this._jsonObject ? new Matrix(this._jsonObject.matrix) : null;

    return cacheResult(this, 'matrix', matrix);
  }
   ...
   */
  get matrixChannels() {
    const matrixChannels = this.matrix === null
      ? []
      : this.allChannels.filter(
          (channel) => channel.pixelKey !== null,
        );

    return cacheResult(this, 'matrixChannels', matrixChannels);
  }
  ...
  /**
   * @returns {string[]} Switching channel aliases that switch to null, ordered by appearance.
   */
  get nullSwitchingChannelAliases() {
    const nullAliases = this.capabilities.flatMap(
      (capability) => Object.entries(capability._jsonObject.switchChannels ?? {}).flatMap(
        ([alias, channelKey]) => (channelKey === null ? [alias] : []),
      ),
    );
    const deduplicatedNullAliases = [...new Set(nullAliases)];

    return cacheResult(this, 'nullSwitchingChannelAliases', deduplicatedNullAliases);
  }

  /**
   * @param {string} alias - The switching channel alias.
   * @returns {NullChannel | null} The null channel for the alias, or null if it does not switch to null.
   */
  getNullChannelForSwitchingAlias(alias) {
    const aliasIndex = this.nullSwitchingChannelAliases.indexOf(alias);
    if (aliasIndex === -1) {
      return null;
    }

    const modeNullChannelCount = Math.max(...this.modes.map((mode) => mode.nullChannelCount));
    return this.nullChannels[modeNullChannelCount + aliasIndex];
  }
  ...
  get nullChannels() {
    ...
    const modeNullChannelCount = Math.max(...this.modes.map((mode) => mode.nullChannelCount));
    const maxNullPerMode = modeNullChannelCount + this.nullSwitchingChannelAliases.length;
    const nullChannels = Array.from({ length: maxNullPerMode }, (_, index) => new NullChannel(this, index + 1));
