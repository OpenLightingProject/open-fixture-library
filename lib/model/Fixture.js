...
module.exports = class Fixture {
  ...
  // returns object whose keys are all items this.allChannelKeys
  // { 'channel key': 'unique channel name' }
  get uniqueChannelNames() {
    if (!('uniqueChannelNames' in this._cache)) {
      this._cache.uniqueChannelNames = {};

      let names = this.allChannels.map(ch => {
        if (ch instanceof FineChannel) {
          return `${ch.coarseChannel.name} fine` + (ch.fineness > 1 ? `^${ch.fineness}` : '');
        }
        else if (ch instanceof SwitchingChannel) {
          return ch.key;
        }
        else {
          return ch.name;
