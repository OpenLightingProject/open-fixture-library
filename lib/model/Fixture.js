...
module.exports = class Fixture {
  ...
  get usedMatrixChannels() {
    if (!('usedMatrixChannels' in this._cache)) {
      ...
      for (const mode of this.modes) {
        for (const templateChannelKey of Object.keys(mode.usedMatrixChannels)) {
          if (!(templateChannelKey in usedMatrixChannels)) {
            usedMatrixChannels[templateChannelKey] = mode.usedMatrixChannels[templateChannelKey];
          }
          else {
            usedMatrixChannels[templateChannelKey] = new Set([
              ...usedMatrixChannels[templateChannelKey],
              ...mode.usedMatrixChannels[templateChannelKey]
            ]);
          }
        }
      }
  ...
  get matrixChannels() {
    if (!('matrixChannels' in this._cache)) {
      ...
      for (const templateChannelKey of Object.keys(this.usedMatrixChannels)) {
        ...
        if (templateChannel !== null) {
          for (const pixelKey of this.usedMatrixChannels[templateChannelKey]) {
