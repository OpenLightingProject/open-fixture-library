...
module.exports = class Mode {
  ...
  _getMatrixChannelKeys(chReference) {
    if (chReference.repeatFor === 'eachPixel') {
      chReference.repeatFor = Object.keys(this.fixture.matrix.pixelKeyPositions);
    }
    else if (chReference.repeatFor === 'eachPixelGroup') {
      chReference.repeatFor = this.fixture.matrix.pixelGroupKeys;
    }

    let channelKeys = [];
    const firstOrder = chReference.channelOrder === 'perPixel' ? chReference.repeatFor : chReference.templateChannels;
    const secondOrder = chReference.channelOrder === 'perPixel' ? chReference.templateChannels : chReference.repeatFor;
    for (const block of firstOrder) {
      for (const item of secondOrder) {
        const pixelKey = chReference.channelOrder === 'perPixel' ? block : item;
        const templateChannel = chReference.channelOrder === 'perPixel' ? item : block;
        channelKeys.push(templateChannel.replace('$pixelKey', pixelKey));
      }
    }
