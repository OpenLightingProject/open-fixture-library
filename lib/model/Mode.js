...
/** @import { SwitchingChannelBehavior } from './SwitchingChannel.js' */
...
class Mode {
   ...
   * @param {Readonly<object>} jsonObject - The mode object from the fixture's JSON data.
   * @param {Readonly<Fixture>} fixture - The fixture this mode is associated to.
   */
  ...
  get physicalOverride() {
    const physicalOverride = 'physical' in this._jsonObject ? new Physical(this._jsonObject.physical) : null;

    return cacheResult(this, 'physicalOverride', physicalOverride);
  }
  ...
  getChannelIndex(channelKey, switchingChannelBehavior = 'all') {
    return this.channels.findIndex((channel) => {
      ...
      return channel instanceof SwitchingChannel
        ? channel.usesChannelKey(channelKey, switchingChannelBehavior)
