...
module.exports = class Mode {
  ...
  /**
   * @returns {!Array.<ChannelReference>} The list of channels used in this mode. The count and position equals the actual DMX channel count and position.
   */
  ...
  /**
   * @returns {!Object.<string, Set<string>>} Each used template channel key pointing to a Set of used pixelKeys/pixelGroupKeys.
   */
  get usedPixelKeys() {
  ...
  _addUsedPixelKey(usedPixelKeys, chRef) {
    if (chRef.isMatrixChannel) {
      ...
      const templateChannel = this.fixture.getTemplateChannelByKey(chRef.templateKey);
      if (templateChannel !== null) {
        for (const switchTo of templateChannel.switchToChannelKeys) {
          this._addUsedPixelKey(usedPixelKeys, ChannelReference.fromMatrixReference(switchTo, chRef.pixelKey));
        }
      }
    }
  }
