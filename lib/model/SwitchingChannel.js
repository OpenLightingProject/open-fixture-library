...
class SwitchingChannel extends AbstractChannel {
   ...
   * @param {Readonly<CoarseChannel>} triggerChannel - The channel whose DMX value this channel depends on.
   */
  ...
  /**
   * @returns {CoarseChannel} The channel whose DMX value this switching channel depends on.
   */
  get triggerChannel() {
  ...
  get triggerCapabilities() {
    return cacheResult(this, 'triggerCapabilities', this.triggerChannel.capabilities.map(
      (capability) => ({
        ...
        switchTo: capability.switchChannels[this.key] === null
          ? this.fixture.getNullChannelForSwitchingAlias(this.key).key
          : capability.switchChannels[this.key],
      }),
    ));
  }
