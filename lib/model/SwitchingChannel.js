module.exports = class SwitchingChannel {
  ...
  // { 'switch to channel key': array of (merged) ranges, ... }
  get triggerRanges() {
    if (!('triggerRanges' in this._cache)) {
      ...
      for (const cap of this.triggerCapabilities) {
        if (!(cap.switchTo in ranges)) {
          ranges[cap.switchTo] = [cap.range];
        }
        else {
          let chRanges = ranges[cap.switchTo];
          if (chRanges[chRanges.length - 1][1] + 1 === cap.range[0]) {
            chRanges[chRanges.length - 1][1] = cap.range[1];
          }
          else {
            chRanges.push(cap.range);
          }
        }
      }
