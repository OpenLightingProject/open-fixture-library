...
export default class Fixture {
  ...
  /**
   * @returns {!string} A globally unique and possibly short product name, defaults to name.
   */
  get shortName() {
  ...
  /**
   * @returns {!Array.<string>} The fixture's categories, ordered by importance (most important first).
   */
  get categories() {
  ...
  /**
   * @returns {!string} The fixture's most important category. Equals the first item of categories.
   */
  get mainCategory() {
  ...
  get fineChannelAliases() {
    if (!(`fineChannelAliases` in this._cache)) {
      this._cache.fineChannelAliases = this._concatArrayProperty(this.availableChannels, `fineChannelAliases`);
    }
  ...
  get fineChannels() {
    if (!(`fineChannels` in this._cache)) {
      this._cache.fineChannels = this._concatArrayProperty(this.availableChannels, `fineChannels`);
    }
  ...
  get switchingChannelAliases() {
    if (!(`switchingChannelAliases` in this._cache)) {
      this._cache.switchingChannelAliases = this._concatArrayProperty(this.availableChannels, `switchingChannelAliases`);
    }
  ...
  get switchingChannels() {
    if (!(`switchingChannels` in this._cache)) {
      this._cache.switchingChannels = this._concatArrayProperty(this.availableChannels, `switchingChannels`);
    }
  ...
  /**
   * @returns {!Array.<string>} Automatically generated null channels.
   */
  get nullChannels() {
  ...
  get capabilities() {
    if (!(`capabilities` in this._cache)) {
      ...
      this._cache.capabilities = this._concatArrayProperty(channels, `capabilities`);
    }
   ...
   */
  _concatArrayProperty(items, property) {
    let values = [];
