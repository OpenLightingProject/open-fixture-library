...
export default class Fixture {
  ...
  get switchingChannelAliases() {
    if (!(`switchingChannelAliases` in this._cache)) {
      this._cache.switchingChannelAliases = concatArrayProperty(this.availableChannels, `switchingChannelAliases`).concat(
        this.matrixChannels.filter(
          matrixChannel => matrixChannel.wrappedChannel instanceof SwitchingChannel
        ).map(
          switchingChannel => switchingChannel.key
        )
      );
    }
  ...
  get switchingChannels() {
    if (!(`switchingChannels` in this._cache)) {
      this._cache.switchingChannels = concatArrayProperty(this.availableChannels, `switchingChannels`).concat(
        this.matrixChannels.map(
          matrixChannel => matrixChannel.wrappedChannel
        ).filter(
          wrappedChannel => wrappedChannel instanceof SwitchingChannel
        )
      );
    }
