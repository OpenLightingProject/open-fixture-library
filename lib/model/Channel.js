  ...
  get capabilities() {
    if (!('capabilities' in this._cache)) {
      if (this.hasCapabilities) {
        const is8Bit = this._jsonObject.capabilities[this._jsonObject.capabilities.length - 1].range[1] === 255;        
        this._cache.capabilities = this._jsonObject.capabilities.map(cap => new Capability(cap, is8Bit, this));
      }
      else {
