...
const namePerType = {
  ...
  ColorPreset: (capability) => {
    ...
    return capability.colorTemperature
      ? appendInBrackets(name, colorTemperaturesToString(capability.colorTemperature))
      : name;
  },
  ...
  Effect: (capability) => {
    ...
    const soundSensitivity = capability.soundSensitivity ? `sound sensitivity ${startEndToString(capability.soundSensitivity)}` : null;
  ...
  Maintenance: (capability) => {
    ...
    const holdString = capability.hold ? `hold ${startEndToString([capability.hold, capability.hold])}` : null;
...
class Capability {
   ...
   * @param {Readonly<object>} jsonObject - The capability data from the channel's JSON.
   * @param {Resolution} resolution - How fine this capability is declared.
   * @param {Readonly<CoarseChannel>} channel - The channel instance this channel is associated to.
   */
   ...
   */
  get name() {
    const name = this.type in namePerType
  ...
  canCrossfadeTo(nextCapability) {
    if (
      this.type !== nextCapability.type
      || this.usedStartEndEntities.length === 0
      || this.usedStartEndEntities.length !== nextCapability.usedStartEndEntities.length
    ) {
      return false;
    }
   ...
   */
  get switchChannels() {
    const switchChannels = this._jsonObject.switchChannels
      ? Object.fromEntries(Object.entries(this._jsonObject.switchChannels).map(
          ([alias, jsonChannelKey]) => {
            const switchedChannelKey = jsonChannelKey ?? this._channel.fixture.getNullChannelForSwitchingAlias(alias).key;
            return [alias, switchedChannelKey];
          },
        ))
      : {};

    return cacheResult(this, 'switchChannels', switchChannels);
  }
   ...
   */
  get hold() {
    const hold = 'hold' in this._jsonObject ? Entity.createFromEntityString(this._jsonObject.hold) : null;

    return cacheResult(this, 'hold', hold);
  }
  ...
  get wheelSlot() {
    ...
    const wheelSlot = this.wheels[0]
      ? this.slotNumber.map(
          (slotNumber) => this.wheels[0].getSlot(slotNumber.number),
        )
      : null;

    return cacheResult(this, 'wheelSlot', wheelSlot);
  }
 ...
 * @param {boolean} [shouldPrependPropertyName=false] - Whether to put the property name before the percentaged value or after.
 * @returns {string} The generated capability name.
 */
function getSimpleCapabilityName(capability, name, property, propertyName = null, shouldPrependPropertyName = false) {
  const propertyString = startEndToString(capability[property], propertyName, shouldPrependPropertyName);
...
function colorTemperaturesToString([start, end]) {
  ...
  function colorTemperatureToString(temperature) {
    ...
    return temperature < 0 ? `${-temperature}% warm` : `${temperature}% cold`;
  }
}
 ...
 */
function startEndToString([start, end], propertyName = null, shouldPrependPropertyName = false) {
  if (start.keyword) {
  ...
  function handleKeywords() {
    ...
    const specifierRegex = / (?:CW|CCW|reverse)$/;
    if (specifierRegex.test(start.keyword) && specifierRegex.test(end.keyword)) {
      const [speedStart, specifierStart] = start.keyword.split(' ', 2);
      const [speedEnd, specifierEnd] = end.keyword.split(' ', 2);
