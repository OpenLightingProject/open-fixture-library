...
const namePerType = {
  ...
  BeamPosition: cap => {
    if (cap.horizontalAngle && cap.verticalAngle) {
      return appendInBrackets(`Beam position (${startEndToString(cap.horizontalAngle)}, ${startEndToString(cap.verticalAngle)})`, cap.comment);
    }

    const orientation = cap.horizontalAngle ? `Horizontal` : `Vertical`;
    const angleStartEnd = cap[`${orientation.toLowerCase()}Angle`];
    ...
    return appendInBrackets(`${prefix} ${startEndToString(angleStartEnd)}`, cap.comment);
  },
  ...
  Maintenance: cap => {
    ...
    let holdString = null;
    if (cap.hold) {
      holdString = `hold ${startEndToString([cap.hold, cap.hold])}`;
    }
...
/**
 * @ignore
 * @param {Capability} cap The capability (with a set slot property) to generate a name for.
...
function getSlotCapabilityName(cap) {
  ...
  return cap.slotNumber[0].number === cap.slotNumber[1].number
    ? cap.wheelSlot[0].name
    : cap.wheelSlot.map(slot => slot.name).join(` … `);
}
