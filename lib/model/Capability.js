...
module.exports = class Capability {
  constructor(jsonObject, specifiedAs8Bit, channel) {
    this.jsonObject = jsonObject; // calls the setter
    this._specifiedAs8Bit = specifiedAs8Bit;
  ...
  get range() {
    if (!('range' in this._cache)) {
      if (this._specifiedAs8Bit) {
        this._cache.range = new Range([
          this._jsonObject.range[0] * Math.pow(256, this._channel.maxFineness),
          (this._jsonObject.range[1] + 1) * Math.pow(256, this._channel.maxFineness) - 1
        ]);
      }
      else {
        this._cache.range = new Range(this._jsonObject.range);
      }
    }
