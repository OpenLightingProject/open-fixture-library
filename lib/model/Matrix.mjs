...
class Matrix {
  ...
  getPixelKeysByOrder(firstAxis, secondAxis, thirdAxis) {
    ...
    const pixelKeys = this.pixelKeys.slice(0); // shallow clone
    return pixelKeys.sort((keyA, keyB) => {
      ...
      if (`pixelGroups` in this._jsonObject) {
        Object.entries(this._jsonObject.pixelGroups).forEach(([groupKey, group]) => {
          if (Array.isArray(group)) {
            ...
            throw new Error(`Pixel key constraints not yet supported for pixel group '${groupKey}'.`);
          }
        });
      }
    }
