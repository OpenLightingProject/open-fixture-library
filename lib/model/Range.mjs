export default class Range {
  ...
  static mergeRanges(ranges) {
    ...
    for (const range of ranges) {
      const mergableRangeIndex = mergedRanges.findIndex(mergedRange => mergedRange.isAdjacentTo(range));

      if (mergableRangeIndex !== -1) {
        mergedRanges[mergableRangeIndex] = mergedRanges[mergableRangeIndex].mergeWith(range);
      }
      else {
