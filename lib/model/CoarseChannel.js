...
class CoarseChannel extends AbstractChannel {
  ...
  get isInverted() {
    if (!(`isInverted` in this._cache)) {
      const proportionalCaps = this.capabilities.filter(capability => !capability.isStep);
      this._cache.isInverted = proportionalCaps.length > 0 && proportionalCaps.every(capability => capability.isInverted);
    }
