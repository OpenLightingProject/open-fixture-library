...
class CoarseChannel extends AbstractChannel {
  ...
  get color() {
    if (!(`color` in this._cache)) {
      const colorCapability = this.capabilities.find(capability => capability.type === `ColorIntensity`);
      this._cache.color = colorCapability ? colorCapability.color : null;
    }
  ...
  getDefaultValueWithResolution(desiredResolution) {
    ...
    if (!(`defaultValuePerResolution` in this._cache)) {
      ...
      for (let index = 1; index <= this.maxResolution; index++) {
        this._cache.defaultValuePerResolution[index] = scaleDmxValue(rawDefaultValue, this.dmxValueResolution, index);
      }
    }
  ...
  getHighlightValueWithResolution(desiredResolution) {
    ...
    if (!(`highlightValuePerResolution` in this._cache)) {
      ...
      for (let index = 1; index <= this.maxResolution; index++) {
        this._cache.highlightValuePerResolution[index] = scaleDmxValue(rawHighlightValue, this.dmxValueResolution, index);
      }
    }
  ...
  get isInverted() {
    if (!(`isInverted` in this._cache)) {
      ...
      this._cache.isInverted = proportionalCapabilities.length > 0 && proportionalCapabilities.every(capability => capability.isInverted);
    }
  ...
  get canCrossfade() {
    if (!(`canCrossfade` in this._cache)) {
      this._cache.canCrossfade = this.capabilities.length === 1
        ? !this.isConstant && this.type !== `NoFunction`
        : this.capabilities.every(
