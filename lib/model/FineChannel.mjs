...
export default class FineChannel extends AbstractChannel {
   ...
   * @param {!Channel} coarseChannel The coarse (MSB) channel.
   */
   ...
   * @param {!Channel} coarseChannel The new coarse channel.
   */
  ...
  /**
   * @returns {!Channel} The coarse (MSB) channel.
   */
  get coarseChannel() {
   ...
   */
  get coarserChannel() {
    return this.resolution === Channel.RESOLUTION_16BIT ? this.coarseChannel : this.coarseChannel.fineChannels[this.resolution - 3];
  }
   ...
   */
  get name() {
    return `${this.coarseChannel.name} fine${this.resolution > Channel.RESOLUTION_16BIT ? `^${this.resolution - 1}` : ``}`;
  }
