...
class TemplateChannel extends CoarseChannel {
   ...
   * @param {Readonly<object>} jsonObject - The template's JSON data which looks pretty similar to a normal channel's data except that channel aliases must include variables.
  ...
  static resolveTemplateObject(object, variables) {
    return JSON.parse(this.resolveTemplateString(JSON.stringify(object), variables));
  }
  ...
  static resolveTemplateString(string, variables) {
    for (const [variable, value] of (Object.entries(variables))) {
      string = stringReplaceAll(string, `$${variable}`, value);
    }
    return string;
  }
}
