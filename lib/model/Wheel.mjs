...
class Wheel {
  ...
  getSlot(slotNumber) {
    ...
    if (!(splitKey in this._cache.splitSlots)) {
      ...
      this._cache.splitSlots[splitKey] = new WheelSlot(null, this, floorSlot, ceilSlot);
    }
   ...
   * @returns {array.<Slot>} All slots with the given type.
   */
  getSlotsOfType(type) {
    if (!(type in this._cache.slotsOfType)) {
      this._cache.slotsOfType[type] = this.slots.filter(
        slot => slot.type === type
      );
    }

    return this._cache.slotsOfType[type];
  }
}
