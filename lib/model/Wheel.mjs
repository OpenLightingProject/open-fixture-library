import WheelSlot from "./WheelSlot.mjs";
...
class Wheel {
  ...
  /**
   * @returns {string} The type of the Wheel, i.e. the most frequent slot type.
   */
  get type() {
   ...
   */
  get slots() {
    if (!(`slots` in this._cache)) {
      this._cache.slots = this._jsonObject.slots.map(slotJson => new WheelSlot(slotJson, this));
    }

    return this._cache.slots;
  }
  ...
  getSlot(slotNumber) {
    ...
    if (!(splitKey in this._cache.splitSlots)) {
      // split slot
      const floorSlot = this.slots[floorIndex];
      const ceilSlot = this.slots[ceilIndex];

      if (floorSlot.type === `AnimationGoboStart`) {
        this._cache.splitSlots[splitKey] = new WheelSlot({
          type: `AnimationGobo`,
          name: `Animation Gobo`
        }, this);
      }
        ...
        const splitSlot = {
          ...
          floorSlot,
          ceilSlot,
          name: `Split ${floorSlot.name} / ${ceilSlot.name}`
        };

        if (floorSlot.colors && ceilSlot.colors) {
          splitSlot.colors = floorSlot.colors.concat(ceilSlot.colors);
        }

        this._cache.splitSlots[splitKey] = new WheelSlot(splitSlot, this);
      }
    }

    return this._cache.splitSlots[splitKey];
  }
