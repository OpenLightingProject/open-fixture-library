<template>
  <header>
    ...
    <nav @click="onNavClick">
      ...
      <div class="right-nav">
        <NuxtLink to="/fixture-editor" title="Fixture editor">Add fixture</NuxtLink>
        <NuxtLink to="/manufacturers" title="Browse fixtures by manufacturer">Manufacturers</NuxtLink>
        <NuxtLink to="/categories" title="Browse fixtures by category">Categories</NuxtLink>
        <NuxtLink to="/about" title="About the project">About</NuxtLink>
...
// initialize from current route query (Nuxt 3/vue-router 4)
const updateSearchQuery = () => {
  const q = route.query.q;
  searchQuery.value = typeof q === 'string' ? q : Array.isArray(q) ? q[0] ?? '' : '';
};

// keep search bar in sync when the route changes
onMounted(() => {
  updateSearchQuery();
});
const stop = watch(
...
const search = () => {
  ...
  // Also trigger focus when navigating via the search button
  emit('focus-content');
};
</script>
