...
* Use JavaScript features that improve code readability, for example:
  ...
  - Use single quotes for strings (`const str = 'Hello world';`) and [template strings](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) only when interpolation is needed (``const str = `My name is ${name}.`;``)
