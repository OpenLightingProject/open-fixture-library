...
We run our [website](https://open-fixture-library.herokuapp.com/) with an [Express](http://expressjs.com/de/) server, the initialization and routing is done in [`main.js`](../main.js). The main script binds different kinds of content into the website:
...
We use [Heroku](https://www.heroku.com/) for automatic deployment:

- The **master branch** is deployed to https://open-fixture-library.herokuapp.com/. It is refreshed with each commit to master, but only after the [Travis tests](#testing) for the commits have finished.
