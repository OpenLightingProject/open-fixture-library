...
`fixtures/register.json` (which is generated by [`cli/make-register.js`](../cli/make-register.js)) is just an index to make searching specific attributes possible without having to read the whole directory structure, which makes it faster. This file is gitignored.
