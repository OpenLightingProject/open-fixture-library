...
All model classes are located in the [`lib/model/`](../lib/model) directory. When using the model, it usally suffices to import the `fixtureFromRepository` function from `index.js` which returns a `Fixture` instance:
