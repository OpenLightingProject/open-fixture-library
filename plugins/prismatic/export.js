...
function mergeColorChannels(prismaticJson, oflJson) {
    ...
    if (!redChannelName.length || !greenChannelName.length || !blueChannelName.length) {
        return;
    }

    let addColorParameter = false;
    // Check if channels contains always RGB in the same order
    for (let mode of prismaticJson.modes) {
        ...
        for (let i = 0; i < channels.length - 2; i++) {
            if (channels[i] == redChannelName && channels[i + 1] == greenChannelName && channels[i + 2] == blueChannelName) {
    ...
    for (let mode of prismaticJson.modes) {
        for (let channel in mode.channels) {
...
function mergeFineChannels(prismaticJson, oflJson) {
    ...
    for (const [name, channel] of Object.entries(oflJson.availableChannels)) {
        ...
        for (let mode in prismaticJson.modes) {
            let channels = mode.channels;
            for (let i = 0; i < channels.length - 1; i++) {
                // check for 24 bit channel, watch out if we are not at the end of the array
                if (alias24Bit.length && i < channels.length - 2 && channels[i] == name && channels[i + 1] == alias16Bit && channels[i + 2] == alias24Bit) {
                    // Replace the 3 channels with the 24 bit channel
                    channels.splice(i, 3, alias24Bit);
                    break;
                }
                // check for 16 bit
                if (channels[i] == name && channels[i + 1] == alias16Bit) {
                    // Replace the 2 channels with the 16 bit channel
                    channels.splice(i, 2, alias16Bit);
                    break;
                }
            }
        }

        // Remove the original channel from available channels
        delete oflJson.availableChannels[name];
    }
}
...
function processChannels(prismaticJson, oflJson) {
    ...
    for (const [name, channel] of Object.entries(oflJson.availableChannels)) {
        let parameterType = ParameterType.Byte;
        const actualName = channel.name ? channel.name : name;
