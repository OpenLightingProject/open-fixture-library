...
const helpMessage = [
  ...
  `Usage: node ${path.relative(process.cwd(), __filename)} -p <plugin-key> [-p <compare-plugin-key>] [-r <git-ref>] [ -t | <fixture> [<more fixtures>] ]`,
  `Options:`,
  `  --plugin,         -p: Which plugin should be used to output fixtures. Allowed values:`,
  `                        ${plugins.exportPlugins.join(`, `)}`,
  `  --compare-plugin, -p: A plugin from the given git reference (may not exist anymore). Defaults to --plugin.`,
